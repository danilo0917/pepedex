    ...
    <Row className={twMerge('gap-4', className)}>
      {configs?.map((config) => {
        ...
        return (
          ...
          >
            {isCurrent ? (
              <div className="absolute p-0.5 rounded-full right-1 top-1 -translate-y-1/2 translate-x-1/2 z-10 bg-field text-color">
                <Icon heroIconName="check-circle" size="smi" />
              </div>
            ) : null}
            <div className="text-color font-medium">{toPercentString(config.tradeFeeRate, { fixed: 4 })}</div>
