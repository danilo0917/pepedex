...
export default function AprCalcDialog() {
  ...
  return (
    ...
    >
      {({ close: closeDialog }) => (
        <Card
          className="p-8 mobile:p-4 rounded-3xl mobile:rounded-lg w-[min(480px,90vw)] max-h-[80vh] overflow-auto mobile:w-full border-1.5 border-gray bg-light-card-bg shadow-cyberpunk-card"
          size="lg"
        >
          <Row className="justify-between items-center mb-4 mobile:mb-2">
            <div className="mobile:text-base text-xl font-semibold text-color">APR Calculation Method</div>
          ...
          <Col className="gap-2 my-4">
            ...
            {choices.map((choice) => {
              ...
              return (
                <Grid
                  className={`grid-cols-2-auto gap-y-3 py-3 px-5 rounded-xl bg-field ${isCurrent ? 'border-1.5 border-[#D0D0D8]' : 'border-1.5 border-transparent'
                    } transition-colors`}
                  key={choice.title}
                  onClick={() => useConcentrated.setState({ aprCalcMode: choice.aprCalcMethod })}
                >
                  <div className="text-color font-medium">{choice.title}</div>
                  <Icon
                    heroIconName="check-circle"
                    size="smi"
                    className={`justify-self-end ${isCurrent ? '' : 'hidden'} text-color`}
                  />
