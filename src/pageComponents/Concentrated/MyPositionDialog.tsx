...
function MyPositionCardTopInfo({ className }: { className?: string }) {
  ...
  return (
    <Row className={twMerge('flex-wrap bg-field grid-cols-4 py-3 px-4 rounded-xl gap-12 mobile:gap-4', className)}>
      <Grid className="grid-rows-[2em,1fr] items-center grow">
        <div className="mobile:text-sm font-medium text-color h-8">Liquidity</div>
        <div className="mobile:text-md font-medium text-2xl text-color">{toUsdVolume(wholeLiquidity)}</div>
      </Grid>
      <Grid className="grid-rows-[2em,1fr] items-center grow">
        <div className="mobile:text-sm font-medium text-color h-8">Leverage</div>
        <div className="mobile:text-md font-medium text-2xl text-color">
          {targetUserPositionAccount?.leverage.toFixed(2)}x
        </div>
      </Grid>
      <Grid className="grid-rows-[2em,1fr] items-center grow">
        <div className="mobile:text-sm font-medium text-color h-8">Deposit Ratio</div>
        <Col className="mobile:text-md font-medium text-2xl text-color gap-2 mobile:gap-1">
          <RowItem
            prefix={
            ...
            text={<div className="text-color justify-end">{toUsdVolume(baseLiquidity)}</div>}
          />
          <RowItem
            prefix={
              <Row className="items-center gap-2">
            ...
            text={<div className="text-color">{toUsdVolume(quoteLiquidity)}</div>}
          />
        </Col>
      </Grid>
      <Grid className="grid-rows-[2em,1fr] items-center grow">
        <div className="mobile:text-sm font-medium text-color h-8">NFT</div>
...
function MyPositionCardChartInfo({ className }: { className?: string }) {
  ...
  return (
    <Col className={twMerge('bg-field py-3 px-4 rounded-xl gap-4', className)}>
      <Row className="items-center gap-2">
        <div className="mobile:text-sm font-medium text-color">My Position</div>
        <RangeTag positionAccount={targetUserPositionAccount} />
      </Row>
      <Grid className="items-center text-color mobile:text-sm">
      ...
      <div className="font-medium text-color mobile:text-xs">
      ...
      <Row className="items-center flex-wrap gap-2">
        <RowItem
          className="border-1.5 border-[#abc4ff40] text-color text-xs font-normal py-1.5 px-3 rounded-lg"
        ...
        <RowItem
          className="border-1.5 border-[#abc4ff40] text-color text-xs font-normal py-1.5 px-3 rounded-lg"
          prefix={<div className="w-3 h-3 bg-field rounded-full mr-2"></div>}
        >
          Current Price
        </RowItem>
      </Row>
...
function MyPositionCardPendingRewardInfo({ className }: { className?: string }) {
  ...
  return (
    <Col className={twMerge('bg-field py-3 px-4 rounded-xl gap-4', className)}>
      <Row className="items-center gap-2">
        <div className="mobile:text-sm font-medium text-color">Pending Yield</div>
      </Row>
      <Row className="items-center gap-8">
        <div className="font-medium text-2xl text-color">≈{toUsdVolume(totalVolume)}</div>
        <Button
      ...
      >
        <div>
          <div className="mobile:text-sm font-medium text-color mt-2 mb-4">Fees</div>
              ...
              <RowItem
                ...
                suffix={
                  <div
                    ...
                    className="grow overflow-hidden text-ellipsis text-color justify-end text-end"
                  >
                    {toString(amount)}
                  </div>
                }
        ...
        <div>
          <div className="mobile:text-sm font-medium text-color mt-2 mb-4">Rewards</div>
...
function MyPositionCardAPRInfo({ className }: { className?: string }) {
  ...
  return (
    <Col className={twMerge('bg-field py-3 px-4 rounded-xl gap-4', className)}>
      <Row className="items-center gap-2">
        <div className="mobile:text-sm font-medium text-color">Estimated APR</div>
        <ConcentratedModifyTooltipIcon />
        <ConcentratedTimeBasisSwitcher className="ml-auto" />
      </Row>
      <div className="font-medium text-2xl mobile:text-lg text-color">{toPercentString(aprCalc?.apr)}</div>
  ...
  return (
    <Row className={twMerge('justify-between gap-2 flex-wrap', className)}>
      <Row className="items-center gap-2">
        ...
        <div className="font-medium text-2xl mobile:text-xl text-color">
...
export default function MyPositionDialog() {
  ...
  return (
    ...
    >
      {({ close: closeDialog }) => (
        <Card
          className="p-8 mobile:p-4 pb-2 rounded-3xl mobile:rounded-lg w-[min(912px,90vw)] max-h-[80vh] overflow-auto mobile:w-full border-1.5 border-gray bg-light-card-bg shadow-cyberpunk-card"
          size="lg"
        >
          <Row className="justify-between items-center gap-6 mobile:gap-3 mb-6 mobile:mb-3">
            <MyPositionCardHeader className="grow" />
            <Icon className="text-color cursor-pointer" heroIconName="x" onClick={closeDialog} />
          </Row>
...
function MyPositionCardPoolOverview({ className }: { className?: string }) {
  ...
  return (
    <Collapse openDirection="upwards" className="w-full mt-4">
      <Collapse.Body>
        <div className="pb-4">
          <div className={twMerge('bg-field py-3 px-4 rounded-xl gap-4 ', className)}>
            <div className="font-medium text-color mb-4">Pool Overview</div>
            <Grid className="grid-cols-5 mobile:grid-cols-2 gap-8 mobile:gap-4 mobile:text-sm">
              <ColItem
                ...
                text={<div className="text-color">{toPercentString(currentAmmPool?.tradeFeeRate)}</div>}
              />
              <ColItem
                className="gap-1 font-medium"
                prefix={<div className="text-[#abc4ff80] min-w-[4em] mr-1">Liquidity</div>}
                text={<div className="text-color">{toUsdVolume(currentAmmPool?.tvl)}</div>}
              />
              <ColItem
                className="gap-1 font-medium"
                prefix={<div className="text-[#abc4ff80] min-w-[4em] mr-1">24h Volume</div>}
                text={<div className="text-color">{toUsdVolume(currentAmmPool?.volume24h)}</div>}
              />
              <ColItem
                className="gap-1 font-medium"
                prefix={<div className="text-[#abc4ff80] min-w-[4em] mr-1">24h Fee</div>}
                text={<div className="text-color">{toUsdVolume(currentAmmPool?.volumeFee24h)}</div>}
              />
              <ColItem
                className="gap-1 font-medium"
                prefix={<div className="text-[#abc4ff80] min-w-[4em] mr-1">Tick Spacing</div>}
                text={<div className="text-color">{currentAmmPool?.ammConfig.tickSpacing}</div>}
              />
              {/* <ColItem
      ...
      <Collapse.Face>
        {({ isOpen }) => (
          <Row className="border-t-1.5 pt-4 pb-2  mobile:pt-2 mobile:pb-0 border-[#abc4ff40] w-full items-center justify-center text-sm font-medium text-color cursor-pointer select-none">
...
function RangeTag({ positionAccount }: { positionAccount?: UserPositionAccount }) {
  ...
  return positionAccount.inRange ? (
    <Row className="items-center bg-[#142B45] rounded text-xs text-color py-0.5 px-1 ml-2">
