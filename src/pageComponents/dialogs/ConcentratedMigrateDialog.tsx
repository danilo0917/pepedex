...
export default function ConcentratedMigrateDialog({
  ...
  const step1 = (closeDialog: () => void) => (
    <Col className="items-center">
      <Icon size="lg" heroIconName="information-circle" className={`text-color mb-3`} />

      <div className="mb-6 text-center">
        <div className="font-semibold text-xl text-color mb-3">{alertTitle}</div>
        <div className="font-normal text-base text-color">{alertDescription}</div>
      ...
      <div className="self-stretch">
        <Col>
          <Button className="text-color frosted-glass-teal">Migrate</Button>
          <Button className="text-color text-sm -mb-4" type="text" onClick={closeDialog}>
            Not now
          </Button>
        </Col>
      </div>
  ...
  const step2 = (closeDialog: () => void) => (
    <div>
      <div className="relative mb-5">
        <div className="text-color text-lg font-medium mb-3">Migrate to Concentrated Liquidity pool</div>
        ...
        <Icon
          heroIconName="x"
          size="lg"
          className="absolute top-0 right-0 text-color text-sm cursor-pointer"
          onClick={closeDialog}
        />
      </div>
      <DetailPanel
      ...
      {({ close: closeDialog }) => (
        <Card
          className={`p-6 mobile:p-4 rounded-3xl mobile:rounded-lg ${canShowMigrateDetail ? 'w-[min(600px,90vw)]' : 'w-[min(450px,90vw)]'
            } mobile:w-full max-h-[80vh] overflow-auto border-1.5 border-gray bg-light-card-bg shadow-cyberpunk-card transition`}
          size="lg"
        >
...
function DetailPanel({
  ...
  return (
    <Grid className="gap-4">
      ...
      {/* CLMM Pool */}
      <div>
        <div className="text-color font-medium mb-2">CLMM Pool</div>
        <Row className="border-1.5 border-[#abc4ff40] rounded-xl py-2 px-4 justify-between">
          <Row className="gap-2">
            <div className="text-color font-medium">
              {base?.symbol ?? '--'}/{quote?.symbol ?? '--'}
            </div>
            <div className="text-color bg-[#abc4ff1f] text-xs rounded-full py-0.5 px-2 font-medium self-center">
      ...
      <div>
        <Row className="items-center justify-between mb-1">
          <div className="text-color font-medium">Price Range</div>
          <RectTabs
            tabs={[
        ...
        {mode === 'quick' && (
          <Row className="border-1.5 border-[#abc4ff40] rounded-xl py-2 px-4 justify-between">
            <div className="text-color font-medium text-sm">
          ...
          <div>
            <Grid className="grid-cols-2-fr gap-3">
              ...
              >
                ...
                <DecimalInput
                  className="font-medium text-sm text-color flex-grow"
      ...
      <div>
        <Row className="pt-6 items-center gap-4">
          <Col className="relative grow border-1.5 border-[#abc4ff40] rounded-xl p-2 gap-1">
            <div className="absolute -top-7 text-center left-0 right-0 text-sm text-color">Current position</div>
            <Row className="justify-between items-center gap-4">
              <Row className="gap-1.5 items-center">
                <CoinAvatar token={base} size="xs" />
                <div className="text-color text-xs">{base?.symbol ?? '--'}</div>
              </Row>
              <div className="text-color text-xs">
            ...
            <Row className="justify-between items-center gap-4">
              <Row className="gap-1.5 items-center">
                ...
                <div className="text-color text-xs">{quote?.symbol ?? '--'}</div>
              </Row>
              <div className="text-color text-xs">
          ...
          <Col className="relative grow border-1.5 border-[#abc4ff40] border-dashed rounded-xl p-2 gap-1">
            <div className="absolute -top-7 text-center left-0 right-0 text-sm text-color">CLMM Pool</div>
            <Row className="justify-between items-center gap-4">
              <CoinAvatar token={base} size="xs" />
              <div className="text-color text-xs">
          ...
          <Col className="relative grow border-1.5 border-[#abc4ff40] border-dashed rounded-xl p-2 gap-1">
            <div className="absolute -top-7 text-center left-0 right-0 text-sm text-color">Wallet</div>
            <Row className="justify-between items-center gap-4">
              <CoinAvatar token={base} size="xs" />
              <div className="text-color text-xs">
        ...
        {farmInfo && farmInfo.rewards.some((i) => gt(i.userPendingReward, 0)) && (
          <div className="text-color text-sm mt-2">
      ...
      <div>
        <Row className="items-center justify-between mb-1">
          <Row className="items-center gap-2">
            <div className="text-color font-medium">Estimated APR</div>
    ...
    <Row className={twMerge(`gap-2 w-full justify-between text-[#fff] ${clmmInfo ? '' : 'opacity-0'}`, className)}>
      ...
      <Row className={`content-around gap-4`}>
        <Row className="items-center gap-2">
          ...
          ></div>
          <div className="w-18 text-color text-sm mobile:text-xs">Trade Fee</div>
        ...
        {aprInfo?.rewards.map(({ token, apr }, idx) => {
          ...
          return (
            <Row className="items-center gap-2" key={token?.mintString}>
              ...
              ></div>
              <div className="w-18 text-color text-sm mobile:text-xs">{token?.symbol}</div>
...
function ModeItem({
  ...
  return (
    <div
      onClick={onClick}
      className={`relative border-1.5 ${selected ? 'border-[#D0D0D8]' : 'border-[#abc4ff40]'
        } rounded-xl py-3 px-4 bg-field cursor-pointer`}
    >
      {selected && <Icon size="sm" className="absolute right-3 top-3" iconSrc="/icons/migrate-check-icon.svg"></Icon>}
      <div className="font-medium text-base text-color mb-1">{title}</div>
      <div className={`font-normal text-sm  ${selected ? 'text-color' : 'text-[#ABC4FF80]'}`}>{description}</div>
    </div>
  )
}
