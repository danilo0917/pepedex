  ...
  return (
    ...
    >
      ...
      <Row className="grow items-center justify-between flex-wrap">
        <div className="mobile:text-sm text-base text-white font-bold">{wallet.adapter.name}</div>
        {detected && !isMobile && (
          <Badge
...
function SimulateWallet({ onClick }: { onClick?(): void }) {
  ...
  return (
    <Col className="p-6 mobile:py-3 mobile:px-4 flex-grow ring-inset ring-1.5 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-3xl mobile:rounded-xl items-center gap-3 m-8 mt-2 mb-4">
      <div className="mobile:text-sm text-base font-bold text-color">Simulate Wallet Address</div>
      <Input
        className="w-full"
  ...
  return (
    ...
    >
      <Row className="items-center justify-between p-8 mobile:p-4">
        <div className="text-xl mobile:text-lg font-semibold text-color">Connect your wallet to PepeDEX</div>
        <Icon className="text-color cursor-pointer" heroIconName="x" onClick={close} />
      </Row>

      {/* Disclaimer */}
      <div className="grow text-sm mobile:text-xs leading-normal text-gray-300 rounded p-4 mobile:p-2 mobile:px-4 mb-6 mobile:mb-4 mx-8 mobile:mx-6 bg-field">
      ...
      <Row
        className="m-4 text-color justify-center items-center clickable"
      ...
      <div className="py-4 text-color text-center font-medium text-sm border-t-1.5 border-gray">
        New here?{' '}
        <Link href="#" className="text-color">
          Get started on PEPE DEX!
        </Link>
      </div>
    </Card>
  )
