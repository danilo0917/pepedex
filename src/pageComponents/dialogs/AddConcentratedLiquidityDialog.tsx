...
export function AddConcentratedLiquidityDialog() {
  ...
  return (
    ...
    >
      {({ close: closeDialog }) => (
        <Card
          className="p-8 mobile:p-4 pb-2 rounded-3xl mobile:rounded-lg w-[min(456px,90vw)] mobile:w-full border-1.5 border-gray bg-light-card-bg shadow-cyberpunk-card"
          size="lg"
        >
          <Row className="justify-between items-center mb-6 mobile:mb-3">
            <div className="mobile:text-base text-xl font-semibold text-color">
          ...
          <Col className="gap-2 mobile:gap-1 border-1.5 rounded-xl border-[#abc4ff40] p-2.5 mobile:p-2 mb-4 mobile:mb-2">
            <div className="font-medium text-sm text-color mobile:pb-1">My Position</div>
            <Row className="items-center">
              ...
              <div className="text-color font-medium text-sm mobile:text-xs">
          ...
          <Col className="gap-2 border-1.5 rounded-xl border-[#abc4ff40] p-2.5 mobile:p-2 mb-4 mobile:mb-2">
            <Row className="items-center py-1">
              <div className="font-medium text-sm text-color mr-auto">Selected Range</div>
              <Row>
                <div className="text-[#abc4ff80] text-xs mr-2">current Price</div>
                <div className="text-color font-medium text-xs">
            ...
            <Grid className="grid-cols-2 gap-4 mobile:gap-2">
              <Col className="gap-2 border-1.5 rounded-xl border-[#D0D0D866] items-center py-3">
                <div className="text-color text-sm">Min Price</div>
                <div className="text-color text-xl font-medium">{toString(targetUserPositionAccount?.priceLower)}</div>
                <div className="text-[#abc4ff80] text-xs">
                  {coinQuote?.symbol ?? '--'} per {coinBase?.symbol ?? '--'}
                </div>
              </Col>
              <Col className="gap-2 border-1.5 rounded-xl border-[#D0D0D866] items-center py-3">
                <div className="text-color text-sm">Max Price</div>
                <div className="text-color text-xl font-medium">{toString(targetUserPositionAccount?.priceUpper)}</div>
                <div className="text-[#abc4ff80] text-xs">
                  {coinQuote?.symbol ?? '--'} per {coinBase?.symbol ?? '--'}
                </div>
              </Col>
          ...
          <FadeInStable show={isMeaningfulNumber(coinBaseAmount)}>
            <Col className="gap-2 mobile:gap-1 border-1.5 rounded-xl border-[#abc4ff40] py-2.5 px-3 mb-4">
              <Col className="justify-between">
                {haveAnyToken2022 ? (
                  <Grid className="mb-1 grid-cols-[2.5fr,2fr,2fr] items-center text-xs text-color">
                    <div></div>
                    <Row className="justify-self-end items-center">
                      ...
                      <Tooltip>
                        ...
                        <Tooltip.Panel>
                          <div className="max-w-[200px] space-y-1.5 text-color">
                  ...
                  {[
                    ...
                    <Grid className="grid-cols-[2.5fr,2fr,2fr] items-center" key={token?.mintString}>
                      <Row className="items-center gap-1 overflow-hidden">
                        <div className="font-medium text-color">{token?.symbol}</div>
                        <CoinAvatar size="sm" token={token} />
                        {isToken2022 && <Token2022Badge />}
                      </Row>

                      <div className="justify-self-end font-medium text-xs text-color">
                        {haveAnyToken2022 ? (
                          isToken2022 ? (
                      ...
                      <div className="justify-self-end font-medium text-color overflow-hidden">
              ...
              <Row className="items-center">
                <div className="mr-auto text-sm mobile:text-xs text-color">Total Deposit</div>
                <div className="text-color font-medium text-sm">{toUsdVolume(totalVolume)}</div>
              </Row>
            </Col>
          </FadeInStable>
          ...
          <Row className="flex-col gap-1">
            ...
            <Button
              type="text"
              size={isMobile ? 'sm' : undefined}
              className="text-sm text-color opacity-50 backdrop-filter-none"
              onClick={closeDialog}
            >
              Cancel
            </Button>
...
function InputLocked() {
  return (
    <div className="absolute text-sm flex flex-col border-1.5 border-[#abc4ff40] text-center justify-center items-center p-2 w-full h-full bg-field bg-opacity-80 z-10 rounded-xl">
