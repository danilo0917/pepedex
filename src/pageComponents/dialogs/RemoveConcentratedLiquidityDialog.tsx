  ...
  return (
    ...
    >
      {({ close: closeDialog }) => (
        <Card
          className={twMerge(
            'p-8 mobile:p-4 rounded-3xl w-[min(500px,90vw)] mobile:w-full border-1.5 border-gray bg-light-card-bg shadow-cyberpunk-card',
            className
          )}
          size="lg"
        >
          ...
          <Col className="gap-3 mb-6 mobile:mb-4">
            ...
            <div className="py-3 px-3 ring-1 mobile:ring-1 ring-[#abc4ff40] rounded-xl mobile:rounded-xl ">
              <Col>
                <Row className="flex justify-start items-center text-color font-medium text-sm">
                  <div className="text-base mobile:text-sm">Pending Yield</div>
                </Row>
                <Row className="flex justify-end items-center text-color font-medium text-sm">
                  {/* <AsyncAwait promise={pendingTotal} fallback="calculating">
                    {(v) => <div className="text-lg text-color">{toUsdVolume(v)}</div>}
                  </AsyncAwait> */}
                  <div className="text-lg text-color">{toUsdVolume(pendingTotalVolume)}</div>
                </Row>
                <FadeInStable show={gt(originalCoin1AmountMin, 0) || gt(originalCoin2AmountMin, 0)}>
                  <Row className="flex justify-start gap-1 items-center text-color font-medium text-sm">
                    Minimum Received
                    <Tooltip>
              ...
              <Row className="flex justify-between items-center text-color font-medium text-sm"></Row>
            </div>
          </Col>
          <Row className="flex-col gap-1">
            ...
            <Button
              size={isMobile ? 'sm' : undefined}
              type="text"
              className="text-sm mobile:text-xs text-color opacity-50 backdrop-filter-none"
              onClick={closeDialog}
            >
              Cancel
            </Button>
...
function MinWithdrawAmount({
  ...
  return (
    <Row className={twMerge('w-full justify-between', className)}>
      ...
      <Row className="text-lg text-color items-center">
        {isToken2022(token) ? (
