...
export default function Select<T extends string>({
  ...
  return (
    <div className={twMerge('relative', shrinkToValue(className, [{ open: isOpen }]))}>
      <div
        className={twMerge(
          `py-2 px-4 mobile:px-3 ring-inset ring-1 ring-[rgb(255,255,255)] h-full rounded-xl mobile:rounded-lg invisible ${disabled ? 'opacity-50 pointer-events-none' : ''
          }`,
      ...
      <Collapse
        className={twMerge(
          `absolute z-10 top-0 left-0 ring-inset ring-1 ring-[rgb(255,255,255)] rounded-xl mobile:rounded-lg w-full transition ${isOpen ? 'bg-light-card-bg' : ''
          }`,
            ...
            <Col className="border-t-1.5 border-[rgba(171,196,255,.1)] px-3 py-1">
              {candidateValues.map((candidate) => {
                ...
                return (
                  <Row
                    key={value}
                    className={`mobile:text-xs text-sm font-medium py-1.5 hover:text-[rgb(196,214,255)] text-[rgba(196,214,255,.5)] cursor-pointer ${value === currentValue ? 'text-[rgba(196,214,255)]' : ''
                      } items-center`}
                    onClick={() => {
