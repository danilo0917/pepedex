    ...
    <div className={twMerge('w-full py-1 px-1', className)}>
      <Row className="w-full flex flex-row justify-between">
        <div className="w-full flex flex-row justify-start items-center ">
          <span
            className={twMerge('text-color font-medium text-base mobile:text-sm', titleClassName)}
            style={{ marginRight: 8 }}
          >
            {title ?? 'Amount'}
          </span>
        ...
        <div className="text-lg mobile:text-sm text-color font-medium flex items-center">
...
function PercentTag({
  ...
  return (
    <div
      className={twMerge(
        'text-color font-medium text-sm mobile:text-xs m-0.25 bg-[#04133d80] rounded-lg mobile:rounded-md hover:bg-[#ABC4FF] hover:text-[#1B1659] py-1 px-3 mobile:px-2 clickable',
        className
      )}
      onClick={() => {
