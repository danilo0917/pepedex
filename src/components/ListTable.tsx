...
export default function ListTable<T extends AnyObj>({
  ...
  const renderListTableRowContent = ({ data }: typeof wrapped[number], idx: number) => {
    return type === 'list-table' ? (
      <Grid
        className={twMerge(
          'text-color text-xs font-medium py-4 px-5 -mx-5 items-center',
      ...
      <div
        className={twMerge(
          'bg-field p-3 divide-y divide-[#abc4ff1a]',
          shrinkToValue(rowClassName, [{ index: idx, item: data }])
        )}
        onClick={() => {
          onClickRow?.({ index: idx, item: data })
        ...
        {parsedShowedPropertyNames.map(({ key, label }) => {
          ...
          return (
            <Grid key={label} className={twMerge('grid-cols-2 py-3', itemClassName)}>
              ...
              {/* item */}
              <div key={label} className="text-color text-xs font-medium">
                {itemNode}
              </div>
            </Grid>
          )
  ...
  return type === 'list-table' ? (
    <Card
      className={twMerge(
        'grid bg-light-card-bg border-1.5 border-gray',
    ...
    >
      ...
      <Grid
        className={twMerge(
          'bg-field px-5 rounded-tr-inherit rounded-tl-inherit items-center',
          headersWrapperClassName
        )}
        style={gridTemplateStyle}
      >
      ...
      {wrapped.map(({ data, destorySelf, changeSelf }, idx) => {
        ...
        return (
          <Card
            ...
            className={twMerge(
              'grid bg-light-card-bg border-1.5 border-gray overflow-hidden',
