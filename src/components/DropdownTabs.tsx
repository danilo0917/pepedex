...
export default function DropdownTabs<T extends string>({
  ...
  const FaceContent = ({
    ...
    <div
      className={`rounded-[22px] mobile:rounded-[18px] p-1 ${($transparentBg && !open) || open ? 'bg-transparent' : 'bg-light-card-bg'
        }`}
    >
      <Row
      ...
      >
        <Row className="min-w-[120px] items-stretch justify-between mobile:min-w-[108px] h-9 mobile:h-7">
          <Row
            onClick={onClickFace}
            className={`grow-2 justify-center items-center ${isValueSelected ? 'text-color' : 'text-color'
  ...
  return (
    <div className={twMerge('relative', className)}>
      ...
      <Collapse
        ...
        className={(open) =>
          `absolute z-dropdown top-0 left-0 w-full ${open ? 'bg-light-card-bg' : 'bg-transparent'
        ...
        <Collapse.Body>
          <RadioGroup
            ...
            itemClassName={(checked) =>
              twMerge(
                `my-3 px-3 text-sm mobile:text-xs font-medium whitespace-nowrap ${checked ? 'text-color' : 'text-color'
                }`,
