...
export default function Switcher({
  ...
  return (
    <Swi
      ...
      className={twMerge(
        `Switch ${isChecked ? (disable ? 'bg-[#7ba4a7]' : 'bg-[#1464ed]') : 'bg-[#1B1659]'
        } relative flex flex-shrink-0 h-5 w-10 mobile:h-4 mobile:w-7 border-1 border-transparent rounded-full ${disable ? 'cursor-not-allowed opacity-20' : 'cursor-pointer'
    ...
    >
      <span
        className={`${isChecked ? 'left-full -translate-x-full' : 'left-0'}
            pointer-events-none absolute top-1/2 h-4 w-4 mobile:h-2.5 mobile:w-2.5 rounded-full ${isChecked ? 'bg-field' : 'bg-[rgba(171,196,255,0.5)]'
