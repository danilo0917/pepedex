...
export default function CoinAvatar({
  ...
  return (
    <>
      ...
      <div
        ref={domRef}
        className={`CoinAvatar flex items-center  ${isRewardBadge ? 'rounded-full ring-1 ring-[#616A9D] p-1' : ''} ${isRewardEnd ? 'opacity-30 ring-[#7B7EAF]' : ''
          } `}
        style={style}
        onClick={onClick}
      >
        {!haveAnime ? (
          ...
          >
            <Image
              className={`${iconSize} rounded-full overflow-hidden transition-transform transform ${hasOpacity ? 'scale-[.7]' : ''
                }`}
              src={src}
              fallbackSrc="/coins/unknown.svg"
            />
