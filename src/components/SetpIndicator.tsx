        ...
        <Row key={index}>
          ...
          <Col className="items-center">
            {shrinkToValue(renderStepNumber, [info]) || (
              <div
                className={`grid place-items-center h-8 w-8 mobile:h-6 mobile:w-6 text-sm font-medium bg-field rounded-full ${currentStep === info.stepNumber ? 'text-[#F1F1F2]' : 'text-color-secondary'
          ...
          <div className="ml-2">
            {shrinkToValue(renderStepContent, [info]) || (
              <div
                className={`text-sm font-medium ${currentStep === info.stepNumber ? 'text-[#F1F1F2]' : 'text-color-secondary'
                  } pt-1.5`}
              >
                {info.stepContent}
              </div>
