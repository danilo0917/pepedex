...
export default function CoinInputBox({
  ...
  return (
    <Row
      className={twMerge(
        `relative flex-col border-solid border-1 border-[#236337] bg-field bg-[#052710] cursor-text rounded-xl py-3 px-6 mobile:px-4 ${disabled && !noDisableStyle ? 'pointer-events-none-entirely cursor-default opacity-50' : ''
        }`,
        className
      )}
      style={style}
    ...
    >
      ...
      <Row className="justify-between mb-2">
        <div className="text-xs mobile:text-2xs text-color" title={HTMLTitleTooltip}>
          {topLeftLabel}
        </div>
        <div
          className={`text-xs mobile:text-2xs justify-self-end text-color ${disabledInput ? '' : 'clickable no-clickable-transform-effect clickable-filter-effect'
            }`}
          onClick={() => {
      ...
      <Row className="col-span-full items-center">
        ...
        <Row className="justify-between flex-grow-2">
          <Row className="gap-px items-center mr-2">
            {!hideMaxButton && (
              <Button
                ...
                className="py-0.5 px-1.5 rounded text-color font-bold bg-max bg-opacity-80 text-xs mobile:text-2xs transition"
                onClick={() => {
                  if (onCustomMax) {
                    onCustomMax()
                    setInputedAmount(typeof maxValue === 'string' ? maxValue : toString(maxValue))
            ...
            {haveHalfButton && (
              <Button
                disabled={disabledInput}
                className="py-0.5 px-1.5 rounded text-color font-bold bg-min bg-opacity-80 text-xs mobile:text-2xs transition"
                onClick={() => {
          ...
          <DecimalInput
            className="font-medium text-lg text-color flex-grow w-full"
            disabled={disabledInput}
            ...
            onEnter={onEnter}
            inputClassName="text-right mobile:text-sm font-medium text-color"
            onBlur={(input) => {
              isOutsideValueLocked.current = false
      ...
      <Row className="items-center gap-2 justify-end text-xs mobile:text-2xs text-color ">
        {/* token 2022 fee */}
        {transferFeeInfo && (
