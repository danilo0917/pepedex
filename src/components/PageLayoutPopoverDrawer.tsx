...
export default function PageLayoutPopoverDrawer({
  ...
  renderPopoverContent?:
    | ReactNode
    | ((controller: { close: () => void }) => ReactNode);
  ...
  const shouldBeDrawer = useMemo(
    () =>
      alwaysPopper == null && alwaysDrawer == null
        ? isMobile
        : !alwaysPopper && alwaysDrawer,
    ...
    >
      ...
      <Popover.Panel>
        {({ close, placement, locationInfo }) => (
            <SmartBubblePanel
              arrowLocationInfo={locationInfo}
...
function SmartBubbleDrawerPanel({ children }: { children?: ReactNode }) {
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(139.48deg, rgba(0, 255, 96, 0.15) 1.07%, rgba(27, 89, 32, 0.1) 86.75%), linear-gradient(321.17deg, #134018 0%, #165918 98.97%)",
      }}
    >
      {children}
    </div>
...
function SmartBubblePanel({
  ...
  arrowLocationInfo?: Pick<
    PopupLocationInfo,
    "arrowLeftRelativeToPanel" | "arrowTopRelativeToPanel"
  >;
}) {
  ...
  const inlineSVG = useMemo(() => {
    ...
    return arrowPosition === "left"
      ? `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg'><rect rx='20' x='8' width='calc(100% - 9px)' height='100%'/>${
          arrowTop && isArrowVisiable
            ? `<polygon points='8,${arrowTop - 8} 0,${arrowTop} 8,${
                arrowTop + 8
              }' />`
            : ""
        }</svg>")`
      ...
      ? `url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg'><rect rx='20' x='0' y='8' width='100%' height='calc(100% - 9px)'/>${
          arrowLeft && isArrowVisiable
            ? `<polygon points='${arrowLeft + 8},8 ${arrowLeft},0 ${
                arrowLeft - 8
              },8' />`
            : ""
        }</svg>")`
  ...
  return (
    <div
      style={{
        filter:
          "drop-shadow(0px 8px 48px rgba(96, 255, 171, 0.12)) drop-shadow(0px 0px 2px rgba(96, 255, 171, 0.5))",
      }}
    >
      <div
        className={twMerge(
