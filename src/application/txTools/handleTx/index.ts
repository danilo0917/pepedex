...
export type TransactionCollector = {
  add(
    transaction: TransactionQueue | Transaction | InnerSimpleTransaction | VersionedTransaction,
...
function collectTxOptions(
  ...
  const innerTransactions = [] as (Transaction | InnerSimpleTransaction | VersionedTransaction)[]
  const { additionalSingleOptionCallback, additionalMultiOptionCallback } = additionOptions ?? {}

  /**
   * mutable
   */
  const addSingle = (transaction: Transaction | InnerSimpleTransaction | VersionedTransaction, options?: SingleTxOption) => {
...
async function dealWithMultiTxOptions({
  ...
  transactions: (Transaction | InnerSimpleTransaction | VersionedTransaction)[]
