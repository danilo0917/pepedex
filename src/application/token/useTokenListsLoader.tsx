...
export function toSplToken(symbol: string, name: string, mintString: string, decimals: number, icon?: string): SplToken {
  const mint = new web3.PublicKey(mintString)
  const token = new Token(mint, mint, decimals, name, symbol)
  return token as SplToken
}
