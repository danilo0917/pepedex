...
const {
  ContextProvider: LiquidityUIContextProvider,
  useStore: useLiquidityContextStore,
} = createContextStore({
...
export default function Liquidity() {
  return (
    <LiquidityUIContextProvider>
      <LiquidityEffect />
      <PageLayout
        mobileBarTitle="Liquidity"
        metaTitle="Liquidity - Pepedex"
      >
        ...
        <div className="ResponsiveFlexLayout">
          {/* <div className="LeftColumn">
            <div className="flex justify-center mb-4">
              <Image src={ElonCat_Swap} width="256" height="150" alt="Pepedex" />
            </div>
            <div className="flex text-center justify-center">
              <h1 className="text-6xl">Pepedex Liquidity</h1>
            </div>
            <br />
...
function useLiquidityWarning() {
  ...
  const [userConfirmedList, setUserConfirmedList] = useLocalStorageItem<
    HexAddress /* ammId */[]
    ...
    () =>
      Boolean(
        useLiquidity.getState().currentJsonInfo &&
          userConfirmedListRef.current?.includes(
            useLiquidity.getState().currentJsonInfo!.id
          )
      ),
    []
  );
  ...
  // permanent state
  const [hasUserPermanentConfirmed, setHasUserPermanentConfirmed] =
    useState(checkPermanent);
  // temporary state
  const [hasUserTemporaryConfirmed, setHasUserTemporaryConfirmed] =
    useState(false);
  ...
  const toggleTemporarilyConfirm = (checkState: boolean) =>
    setHasUserTemporaryConfirmed(checkState);
...
function ConfirmRiskPanel({
  ...
  return (
    <div
      className={twMerge(
        "bg-tooltip rounded-xl py-3 px-6 mobile:px-4",
        className
      )}
    >
      ...
      <Checkbox
        ...
        label={<div className="text-sm italic text-gray-300">Confirm</div>}
      />
      ...
      <Checkbox
        ...
        label={
          <div className="text-sm italic text-gray-300">
            Do not warn again for this pool
          </div>
        }
      />
    </div>
...
function LiquidityCard() {
  ...
  const [
    isCoinSelectorOn,
    { on: turnOnCoinSelector, off: turnOffCoinSelector },
  ] = useToggle();
  // it is for coin selector panel
  const [targetCoinNo, setTargetCoinNo] = useState<"1" | "2">("1");

  const checkWalletHasEnoughBalance = useWallet(
    (s) => s.checkWalletHasEnoughBalance
  );
  ...
  const isHydrating = useMemo(
    () => currentJsonInfo && !currentHydratedInfo,
    [currentJsonInfo, currentHydratedInfo]
  );
  const hasHydratedLiquidityPool = useMemo(
    () => Boolean(currentHydratedInfo),
    [currentHydratedInfo]
  );
  ...
  const haveEnoughCoin1 =
    ...
    checkWalletHasEnoughBalance(
      toTokenAmount(
        coin1,
        focusSide === "coin1" ? coin1Amount : unslippagedCoin1Amount,
  ...
  const haveEnoughCoin2 =
    ...
    checkWalletHasEnoughBalance(
      toTokenAmount(
        coin2,
        focusSide === "coin2" ? coin2Amount : unslippagedCoin2Amount,
        { alreadyDecimaled: true }
      )
    );
  ...
  const poolIsOpen =
    currentHydratedInfo &&
    isDateAfter(currentTime, currentHydratedInfo.startTime);
  function getDurationText(val: number) {
    const duration = parseDuration(val);
    return `Pool Opens in ${String(duration.days).padStart(2, "0")}D : ${String(
      duration.hours
    ).padStart(2, "0")}H : ${String(duration.minutes).padStart(2, "0")}M`;
  }
  const remainTimeText =
    currentHydratedInfo && !poolIsOpen
      ? getDurationText(currentHydratedInfo?.startTime - currentTime)
      : undefined;
  return (
    <CyberpunkStyleCard
      domRef={cardRef}
    ...
    >
      ...
      <>
        <CoinInputBox
          ...
          onEnter={(input) => {
            ...
            if (coin2 && coin2Amount)
              liquidityButtonComponentRef.current?.click?.();
          }}
          token={coin1}
        />
        ...
        <div className="relative h-8 my-4">
          ...
          >
            <Icon heroIconName="plus" className="p-1 text-white" />
            <FadeIn>
              {hasHydratedLiquidityPool && (
          ...
          <Row className="absolute right-0 items-center ">
            <Icon
              size="sm"
              heroIconName="search"
              className={`p-2 frosted-glass -translate-y-2 frosted-glass-teal rounded-full mr-4 clickable text-white select-none ${
            ...
            />
            <div
              className={isApprovePanelShown ? "not-clickable" : "clickable"}
            >
              <RefreshCircle
      ...
      {/* supply button */}
      <Button
        className="connect-btn w-full mt-5"
        ...
        validators={[
          ...
          {
            should:
              (coin1Amount && isMeaningfulNumber(coin1Amount)) ||
              (coin2Amount && isMeaningfulNumber(coin2Amount)),
          ...
          },
          {
            ...
            fallbackProps: {
              children: `Insufficient ${coin1?.symbol ?? ""} balance`,
            },
          },
          {
            should: haveEnoughCoin2,
            fallbackProps: {
              children: `Insufficient ${coin2?.symbol ?? ""} balance`,
            },
          },
        ]}
        onClick={() => {
        ...
        onSelectToken={(token) => {
          if (targetCoinNo === "1") {
            if (!areSameToken(coin1, token)) {
              useLiquidity.setState({
                coin1: token,
                isCalculatingBczSelection: true,
              });
              // delete other
...
function canTokenPairBeSelected(
  targetToken: SplToken | undefined,
  candidateToken: SplToken | undefined
) {
  return !isMintEqual(targetToken?.mint, candidateToken?.mint);
}

function areSameToken(
  originToken: SplToken | undefined,
  newSelected: SplToken
): boolean {
          ...
          <Tooltip placement="bottom-right">
            ...
            <Tooltip.Panel>
              <p className="w-80">
                ...
                {SOL_BASE_BALANCE} SOL is recommended to avoid failed
                transactions. This swap will leave you with less than{" "}
                {SOL_BASE_BALANCE} SOL.
              </p>
            </Tooltip.Panel>
          </Tooltip>
        </Row>
...
function LiquidityCardPriceIndicator({ className }: { className?: string }) {
  ...
  const pooledBaseTokenAmount = currentHydratedInfo?.baseToken
    ? toTokenAmount(
        currentHydratedInfo.baseToken,
        currentHydratedInfo.baseReserve
      )
    : undefined;
  const pooledQuoteTokenAmount = currentHydratedInfo?.quoteToken
    ? toTokenAmount(
        currentHydratedInfo.quoteToken,
        currentHydratedInfo.quoteReserve
      )
    : undefined;

  const isCoin1Base =
    String(currentHydratedInfo?.baseMint) === String(coin1?.mint);
  ...
  const isStable = useMemo(
    () => Boolean(currentHydratedInfo?.version === 5),
    [currentHydratedInfo]
  );
  ...
  if (isStable) {
    ...
    return (
      <Row
        className={twMerge(
          "font-medium text-sm mobile:text-xs text-color",
          className
        )}
      >
        <div className="flex justify-start align-middle">
          <div className="flex justify-start m-auto text-2xl mobile:text-lg align-middle pb-1">
            {"{"}&nbsp;
          </div>
...
function useLpBurnInfo({
  ...
  const realSupplyAmount = lpMintInfo
    ? (MintLayout.decode(lpMintInfo?.data).supply as bigint)
    : undefined;
  const _allLpAmount = getMax(realSupplyAmount ?? 0, minus(allLpAmount, 1));
  const burnAmount =
    realSupplyAmount != null
      ? minus(_allLpAmount, realSupplyAmount)
      : undefined;
    ...
    >
      <Col className="w-full">
        <LiquidityCardItem
          ...
          fieldValue={
            focusSide === "coin1"
              ? coin1?.symbol ?? "unknown"
              : coin2?.symbol ?? "unknown"
          }
        />
        <FadeIn>
          ...
          fieldValue={
            <div>
              {pooledBaseTokenAmount
                ? `${formatNumber(pooledBaseTokenAmount.toExact())} ${
                    coinBase?.symbol ?? "unknown"
                  }`
                : "--"}
            </div>
          }
          ...
          fieldValue={
            <div>
              {pooledQuoteTokenAmount
                ? `${formatNumber(pooledQuoteTokenAmount.toExact())} ${
                    coinQuote?.symbol ?? "unknown"
                  }`
                : "--"}
            </div>
          }
              ...
              <Row className="gap-1">
                ...
                {gte(burnAmountPercent, 0.01) && (
                  <Row className="text-xs items-center">
                    <div>
                      ({toPercentString(burnAmountPercent, { fixed: 1 })} Burned
                    </div>
                    <Tooltip>
                      <Icon
                        iconClassName="ml-1"
                      ...
                      <Tooltip.Panel>
                        <div className="max-w-[300px]">
                          {toPercentString(burnAmountPercent, { fixed: 1 })} of
                          the LP token supply displayed has been burned and
                          can’t be removed from the pool. This figure does not
                          take into account the percent of token supply used to
                          add liquidity.
                        </div>
                      </Tooltip.Panel>
                    </Tooltip>
        ...
        <FadeIn>
          {currentHydratedInfo && !poolIsOpen ? (
            <LiquidityCardItem
              fieldName="Open at"
              fieldValue={<div>{toUTC(currentHydratedInfo.startTime)}</div>}
            />
          ) : undefined}
        </FadeIn>
        <Collapse openDirection="upwards" className="w-full">
          <Collapse.Body>
            <Col>
              <LiquidityCardItem
                fieldName="Addresses"
                tooltipContent={<LiquidityCardTooltipPanelAddress />}
              />
              <LiquidityCardItem
...
function LiquidityCardItem({
  ...
  return (
    <Row className={twMerge("w-full justify-between my-1.5", className)}>
      ...
      <div className="text-xs font-medium text-color text-right">
        {fieldValue}
      </div>
    </Row>
  );
  ...
  return (
    <div className="w-60">
      ...
      <Col className="gap-2">
        ...
        {Boolean(id) && (
          <LiquidityCardTooltipPanelAddressItem label="Amm ID" address={id!} />
        )}
        {Boolean(marketId) && (
          <LiquidityCardTooltipPanelAddressItem
            label="Market ID"
...
function LiquidityCardTooltipPanelAddressItem({
  ...
  return (
    <Row
      className={twMerge(
        "grid gap-2 items-center grid-cols-[5em,1fr,auto,auto]",
        className
      )}
    >
...
function UserLiquidityExhibition() {
  ...
  const userExhibitionLiquidityIds = useLiquidity(
    (s) => s.userExhibitionLiquidityIds
  );
  ...
  const exhibitionInfos = useMemo(
    () =>
      hydratedInfos.filter(({ id }) =>
        userExhibitionLiquidityIds?.includes(toPubString(id))
      ),
    [hydratedInfos, userExhibitionLiquidityIds]
  );

  const isMigrateToClmmDialogOpen = useConcentrated(
    (s) => s.isMigrateToClmmDialogOpen
  );
  ...
  return (
    <div className="mt-12 max-w-[456px] self-center">
      ...
      <Card
        className="p-6 mt-6 mobile:py-5 mobile:px-3 bg-light-card-bg"
        size="lg"
      >
        <List
          items={exhibitionInfos}
          getItemKey={(info) => toPubString(info.id)}
          className={`flex flex-col gap-6 mobile:gap-5 ${
            exhibitionInfos.length ? "mb-5" : ""
          }`}
        >
          {(info) => {
            const canMigrate = migrationJsonInfo?.some(
              (m) => m.ammId === toPubString(info.id)
            );
            return (
              <FadeIn>
                        ...
                        <Row className="justify-between">
                          ...
                          <div className="text-xs mobile:text-2xs font-medium text-color">
                            {info.lpMint
                              ? toString(
                                  div(
                                    rawBalances[String(info.lpMint)],
                                    10 ** info.lpDecimals
                                  ),
                                  {
        ...
        <div className="text-xs mobile:text-2xs font-medium text-gray-300">
    ...
    <div className="mt-12 max-w-[456px] self-center">
      ...
      >
        <Row className="gap-4">
          ...
          <Button
            className="items-center connect-btn opacity-80"
            onClick={() => {
              routeTo("/liquidity/create");
            }}
          >
            
            <div className="flex flex-row"><Icon className="mr-2 mobile:hidden" heroIconName="plus" />Create Pool</div>
          </Button>
        </Row>
      </Card>
    </div>
